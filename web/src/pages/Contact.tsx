  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = body?.errors?.[0]?.message || body?.message || 'Failed to send message'
        throw new Error(msg)
      }
      setStatus('sent')
      setName('')
      ...
      <main className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed text-[var(--muted-foreground)] max-w-md mb-8">
              We are committed to attending to any of your enquiries about BLOCKLIFT
              and try to provide the best services.
            </p>
            <ul className="space-y-6 text-sm">
              <li className="flex items-center gap-4">
                <span className="p-2 rounded-md border border-primary text-primary"><Mail className="h-4 w-4" /></span>
                <a
              ...
              <li className="flex items-center gap-4">
                <span className="p-2 rounded-md border border-primary text-primary"><WhatsAppIcon className="h-4 w-4" /></span>
                <a
              ...
              <li className="flex items-center gap-4">
                <span className="p-2 rounded-md border border-primary text-primary"><Phone className="h-4 w-4" /></span>
                <a
          ...
          <div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {status === 'sent' && (
                <p className="text-xs text-emerald-500">Thanks! We received your message.</p>
              )}
            </form>
          </div>
        </div>
