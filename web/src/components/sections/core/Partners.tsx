...
const partners = [
  {
    ...
    metric: "multiple Students Reached",
...
export default function Parteners() {
  return (
    <section
      id="partners"
      tabIndex={-1}
      className="w-full text-foreground py-12 mb-12"
    >
      <div className="max-w-7xl mx-auto px-6">
        <p className="text-md md:text-xl font-light text-center text-primary">
          Impact Network
        </p>
        <h2 className="text-4xl md:text-5xl font-semibold text-center mb-8 md:mb-12">
          Partners
        </h2>
            ...
            <Card key={i} className="px-6 py-8">
              <CardHeader className="flex items-center gap-4">
                ...
                {p.name.includes("Let Africa Build") ? (
                  <img
                    src={labLogo}
                    alt="Let Africa Build logo"
                    className="w-14 h-14 object-contain rounded-md bg-transparent"
                  />
              ...
              <CardFooter className="flex justify-between">
                {p.xUrl ? (
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      window.open(p.xUrl, "_blank", "noopener,noreferrer")
                    }
                  >
                    View Partner
                  </Button>
